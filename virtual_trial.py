...
def process_frame(frame, file_path):
    ...
    try:
        ...
        logging.error(f"Error initializing images and photos: {e}")
    ...
    for face in faces: 
        ...
        logging.info(f"Face detected at: {face.left()}, {face.top()}, {face.width()}, {face.height()}")
        ...
        if SPRITES[3]:  # Tiara
            logging.debug("Applying Tiara sprite")
            apply_sprite(frame, IMAGES[3][ACTIVE_IMAGES[3]],w+45,x-20,y+20, incl, ontop = True)
            sprite_applied = True

        # Necklaces
        if SPRITES[1]:  # Necklaces
            logging.debug("Applying Necklace sprite")
        ...
        # Goggles
        if SPRITES[6]:  # Goggles
            logging.debug("Applying Goggles sprite")
        ...
        if SPRITES[2]:  # Earrings
            logging.debug("Applying Earrings sprite")
    ...
    if not sprite_applied:
        logging.warning("No sprites applied to the frame.")
